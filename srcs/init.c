...
/*   Updated: 2025/08/09 18:21:39 by adrmarqu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_data	init_data(char **env)
{
	...
	if (!ret.env)
		return (ret);
	ret.level = get_level(ret.env);
	if (add_hidden(ret.level, ret.env))
		return (ret);
	ret.end = false;
	curr = ret.env;
	while (curr)
	{
		if (!ft_strcmp(curr->var, "HOME"))
		{
			ret.home = ft_strdup(curr->value);
			return (ret);
		}
		curr = curr->next;
	}
